...
Clipboard::Clipboard(QWidget *parent) : QWidget(parent)
{
    ...
    struct stat st;
    if(::stat(curDir.toUtf8().data(), &st) != 0)
        mkdir(curDir.toUtf8().data(), S_IRWXU);
...
void Clipboard::onHasFileData(QByteArray& data)
{
    if(!fp)
    {
        ...
        fp = fopen(fileUriToPaste.toUtf8().data(), "wb");
        return;
    }
    if(data.size() == 0)
    {
        ...
        if(!hasDir)
        {
            ...
            qDebug() << uri;
            self = true;
            mime->setData("x-special/gnome-copied-files", uri.toUtf8());
...
void Clipboard::onHasDirInfo(QString dir)
{
    if(dir.length() != 0)
    {
        ...
        dirs.push(curDir);
        ::mkdir(newDir.toUtf8().data(), S_IRWXU);
        curDir = newDir;
    }
    else
    {
        ...
        if(dirs.empty())
        {
            ...
            QMimeData* mime = new QMimeData;
            self = true;
            mime->setData("x-special/gnome-copied-files", dirUriToPaste.toUtf8());
            board->setMimeData(mime);
        }
    }
}
